...
def _workshop_date_context():
    """Return min/max ISO date strings for the propose-workshop date input."""
    today = date.today()
    return {
        'min_date': (today + timedelta(days=3)).isoformat(),
        'max_date': (today + timedelta(days=365)).isoformat(),
    }
...
def activate_user(request, key=None):
    ...
    if key is None:
        if user.is_authenticated and not user.profile.is_email_verified and \
            ...
            return render(request, 'workshop_app/activation.html', {'status': "2"})
        else:
    ...
    user.save()
    return render(request, 'workshop_app/activation.html', {"status": "0"})
...
def workshop_status_instructor(request):
    """Workshops for instructor: pending proposals + accepted upcoming."""
    user = request.user
...
def change_workshop_date(request, workshop_id):
    ...
    if request.method == 'POST':
        ...
        if today <= new_workshop_date:
            ...
            messages.error(request, "New date must be today or later.")
    return redirect(reverse('workshop_app:workshop_status_instructor'))


@login_required
def propose_workshop(request):
    """Coordinator proposes a workshop and date."""
    user = request.user
    if user.is_superuser:
        return redirect("/admin")
    if is_instructor(user):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if Workshop.objects.filter(
                ...
                messages.warning(request, "You have already proposed this workshop on this date.")
                return redirect(get_landing_page(user))
            else:
                form_data.save()
    ...
    ctx = {"form": form}
    ctx.update(_workshop_date_context())
    return render(request, 'workshop_app/propose_workshop.html', ctx)


@login_required
def workshop_type_details(request, workshop_type_id):
    """Workshop type details — editable by instructors."""
    user = request.user
    if user.is_superuser:
        return redirect("/admin")
    ...
    if is_instructor(user):
        if request.method == 'POST':
            ...
            if form.is_valid():
                form_data = form.save()
                messages.success(request, "Workshop type saved.")
                for file in form_file:
                    if (file.is_valid() and file.clean() and
                        ...
                        file.save()
                        messages.info(request, "Attachment saved.")
                return redirect(
...
def workshop_type_list(request):
    ...
    return render(
        request, 'workshop_app/workshop_type_list.html',
        {'workshop_type': workshop_type}
    )
...
def workshop_details(request, workshop_id):
    ...
    return render(
        request, 'workshop_app/workshop_details.html',
        {
            'workshop': workshop,
...
def add_workshop_type(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form_data = form.save()
            messages.success(request, "Workshop type added.")
            return redirect(
                reverse('workshop_app:workshop_type_details', args=[form_data.id])
            )
    else:
...
def view_profile(request, user_id):
    """Instructor views a coordinator's profile."""
    user = request.user
    if is_instructor(user) and is_email_checked(user):
        ...
        return render(
            request, "workshop_app/view_profile.html",
...
def view_own_profile(request):
    """User views and edits their own profile."""
    user = request.user
    if user.is_superuser:
        return redirect("admin")
